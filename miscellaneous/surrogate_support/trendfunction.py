...
def compute_regression_mat(idx,X,bounds,polytype):
    ...
    for po in range(0,nvar):
        if polytype[po] == 1:
            tempres = 2*((np.transpose([X[:,po]])-np.matlib.repmat(bounds[0,po],nsamp,1)) /(np.matlib.repmat(bounds[1,po],nsamp,1)-np.matlib.repmat(bounds[0,po],nsamp,1)))-1;
            N[:, po] = tempres[:,0]
        elif polytype[po] == 2:
            tempres = (np.transpose([X[:,po]])-np.matlib.repmat(bounds[0,po],nsamp,1))/(np.sqrt(2*np.matlib.repmat(bounds[1,po],nsamp,1)**2))
            N[:, po] = tempres[:,0]
...
def legendre(n,lb,ub):
    ...
    if n == 0:
        ...
        for i in range(0,n-1):
            ...
            a1 = np.polyval(np.polyint(np.convolve(np.array([1,0]),np.convolve(L,L))),ub) - np.polyval(np.polyint(np.convolve(np.array([1,0]),np.convolve(L,L))),lb)
